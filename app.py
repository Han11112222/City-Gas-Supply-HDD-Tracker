...
def load_and_process_data():
    ...
    # [수정됨] 미래 기온 추정: 과거 전체 평균이 아닌, '최근 3년 평균 기온'을 산출하여 대입합니다.
    ...
    if estimate_btn:
        if len(train_range) == 2 and len(predict_range) == 2:
            ...
            if not forecast_df.empty:
                ...
                # [추가됨] 하단 기본 설명 안내 박스
                st.info("💡 **미래 기온 및 공급량 추정 방식 안내:** 미래 예측 기간(실적이 없는 날짜)의 기온은 최근 대구 지역의 기온 상승 트렌드를 모델에 가장 민감하게 반영하기 위해, 과거 전체 기간의 평균이 아닌 **해당 날짜(월-일) 기준 가장 최근 3년 치 실적의 평균 기온**을 별도로 산출하여 예측에 자동 적용하였습니다.")
